    ...
    <el-row :span="24" align='middle'>
      <el-col :span="5">
        <el-form ref="form" label-width="100px" size="mini">
          ...
          <el-form-item label="子序列长度:">
            <el-input v-model="seqlength" style="margin-right: 100px;">
        ...
        <el-card class="box-card" :span="5">
          ...
          <div v-if="newType == '时间窗口'">
            <p>推测的封禁原因为：{{(newParams.newTimeWindow / 60).toFixed(2)}}分钟内,访问次数达到了{{newParams.newTimeBan}}次;在所有访问次数达到{{newParams.newTimeBan}}的访问记录中，有{{(newParams.newNumberMix*100).toFixed(2)}}%的访问被封禁。
            </p>
          </div>
          <div v-if="newType == '次数窗口'">
            <p>推测的封禁原因为：{{(newParams.newNumberWindow)}}次访问的耗时小于{{newParams.newNumberBan}};在所有访问耗时小于{{newParams.newNumberBan}}的访问记录中，有{{(newParams.newNumberMix*100).toFixed(2)}}%的访问被封禁。
            </p>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="19">
        <div id="periodism" style="width: 1000px;height:500px;"></div>
      </el-col>
    </el-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    drawLine: function () {
      ...
      params.append('seqLength', (this.seqlength))
      ...
      api.getPeriodismData(params).then(data => {
        this.lineData = data.lineData_number
        console.log(data)
        this.boxData = data.boxData

        // 2.准备option
        var option = utils.getLineOption(this.featuretype, this.windowtype,
          this.lineData, this.scale, this.selected,
          this.beginTimeStr, this.endTimeStr)
        // 3.获取实例，画图

        var myChart_per = this.$echarts.init(document.getElementById('periodism'));
        console.log('初始化echarts实例,')
        var t1 = new Date()
        myChart_per.setOption(option);
        var that = this
        myChart_per.on('datazoom', function (params) {
          // console.log(params);//里面存有代表滑动条的起始的数字
          var start = myChart_per.getModel().option.dataZoom[0].start;//获取axis
          var end = myChart_per.getModel().option.dataZoom[0].end;//获取axis
          ...
          myChart_per.setOption(that.lineOption)
        })
        myChart_per.on('legendselectchanged', function (params) {
          // console.log(params.selected)
          that.selected = params.selected
          that.rechart++
        })
        var t2 = new Date()
        console.log("---------------------图形渲染用时：", (t2.getTime() - t1.getTime()) / 1000)
      })
    ...
    getPreParam: function () {
      ...
      api.getPeriodismParams(params).then(data => {
