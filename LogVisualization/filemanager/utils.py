...
SUCCESS = ['0', '000000']
SUSPECTED = ['2', '906010', '906011', '906021']
BANNED = ['1' ]
...
2 900001 验证码错误 
3 900002 用户名或密码错误 
4 900003 未登录或登录超时，请重新登录 
5 900004 未输入参数或参数格式有误 
6 900005 没有当前查询信息 
7 900006 原网实际返回的错误信息 
8 900007 输入的页码超出限制 
9 900008 无效的iw-apikey 
10 900009 发送内容过长 
11 900010 API接口维护中或已停用 
12 900011 网络中断或超时，稍后重试 
13 999999 接口请求失败，请联系技术人员 
14 900013 需输入短信码使用 
15 906*** 输入验证码
'''
    ...
    with open(os.path.join(baseDir, 'tmp', fileName)) as f:
        ...
        for log in logs:
            ...
            if(code in BANNED):
                ...
                logs_write.append(log)
            # ++++++++++++++++++++++++++++++++++++++++++++++++ 完成论文更改#
            # if(isinstance(log['operationResult'], str) and 'rtnCode' not in log['operationResult']):
            #     continue
            # if(isinstance(log['operationResult'], str) and 'rtnCode' in log['operationResult']):
            #     # log['operationResult'] = json.loads(log['operationResult'])
            #     # code = log['operationResult'][12:18]
            #     code = json.load(log['operationResult'])['rtnCode']
            #     if(code in SUCCESS):
            #         IS_SUCCESS = 1
            #         log['operationResult'] = code
            #         logs_write.append(log)
            ...
            # +++++++++++++++++++++++++++++++++++++ #
                    # tmp.add(code)
                # ---------------- 分析部分 ------------------------##############
                # if(code in TO_COMPARE):
                #     if(re.search(pattren_rate_limit_exceeded, log['operationResult'])):
                #       IS_BANNED = 1
                #       log['operationResult'] = '1'  
                #       logs_write.append(log)
                      # tmp.add(code)
                    # if(re.search(pattern_unusual_login, log['operationResult'])):
                    #   IS_SUSPECTED = 1
                    #   log['operationResult'] = 2
                    #   logs_write.append(log)
        # print(tmp)
        # del logs[idx_to_delete]
        # print(logs_write[0:5])
        # quit()
        ...
        for log in logs_write:
            ...
            #     log['status']['code'] = 3-log['status']['code']
            # rtnCode兼容测试日志与真实日志
            # print(log['operationResult'])
            # quit()
